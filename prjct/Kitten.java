...
public class Kitten extends Character {
    ...
    private void buildKitten() {
        ...
        for (Node node : kittenGroup.getChildren()) {
            if (node instanceof Shape) {
                if (hitbox == null) {
                    hitbox = (Shape) node;
                } else {
                    hitbox = Shape.union(hitbox, (Shape) node);
                }
            }
        }
    }
    ...
    public boolean check(Obstacle obstacle){
        ...
        for (Node nodeB : group2.getChildren()) {
            if (nodeB instanceof Shape) {
                Shape shapeObstacle = (Shape) nodeB;
                Shape intersection = Shape.intersect(hitbox, shapeObstacle);
